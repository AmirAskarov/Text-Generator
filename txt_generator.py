...
def plot_scores_per_image(plotting_data, output_dir):
    ...
    for img_id, iter_data_list in plotting_data.items():
        if not iter_data_list: # Skip if no data for this image (e.g., it was always skipped)
            continue
...
def run_iteration(clip_model, clip_processor, sgpt_model, output_dir, semantic_descriptions, semantic_sim_matrix, client):
    ...
    for iteration in range(1, ITERATIONS):
        ...
        for img_i in range(1, NUM_IMGS):
            ...
            # Generating new visual description based on the previous and the best 
            if iteration > 1:
            ...
            # Updating the best score according to clip ascore
        ...
        # Saving all the visual descriptions for the current iteration
